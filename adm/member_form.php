...
else if ($w == 'u')
{
	...
	$mb['mb_name_en'] = get_text($mb['mb_name_en']);
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
		...
		<th scope="row"><label for="mb_name_en">영문명<strong class="sound_only">필수</strong></label></th>
        <td><input type="text" name="mb_name_en" value="<?php echo $mb['mb_name_en'] ?>" id="mb_name_en" required class="required frm_input" size="35"  maxlength="20"></td>
    ...
    <tr>
        ...
        <td colspan="3" class="td_addr_line">
			...
			<input type="text" name="mb_addr_eng2" value="<?php echo $mb['mb_addr_eng2'] ?>" id="mb_addr_eng2" class="frm_input" size="60">
			<input type="text" name="mb_addr_eng" value="<?php echo $mb['mb_addr_eng'] ?>" id="mb_addr_eng" class="frm_input" size="60">
			<label for="mb_addr_eng">영문주소</label>
        </td>
    </tr>
	<?php
		$sql = " SELECT * FROM {$g5['g5_shop_order_table']} WHERE mb_id = '". $mb['mb_id'] ."' ";
		$result = sql_query($sql);
		$odCnt = sql_num_rows($result);
		$totalPayPrice = 0;
		$totalExceptDeliveryPrice = 0;
		$totalUsePoint = 0;
		for ($i=0; $row=sql_fetch_array($result); $i++) {
			$totalPayPrice += $row['od_receipt_price'];
			$realPayPrice = $row['od_receipt_price'] - $row['od_send_cost'];
			$totalExceptDeliveryPrice += $realPayPrice;
			$totalUsePoint = $row['od_receipt_point'];
		}
	?>
	<tr>
		<th scope="row">누적 주문건수</th>
		<td colspan="3"><?php echo number_format($odCnt); ?>건</td>
	</tr>
	<tr>
		<th scope="row">총 결제액</th>
		<td colspan="3"><?php echo number_format($totalPayPrice); ?>원</td>
	</tr>
	<tr>
		<th scope="row">배송비 제외</th>
		<td colspan="3"><?php echo number_format($totalExceptDeliveryPrice); ?>원</td>
	</tr>
	<tr>
		<th scope="row">적립금 사용금액</th>
		<td colspan="3"><?php echo number_format($totalUsePoint); ?>원</td>
	</tr>
    <tr>
...
this.form.mb_intercept_date.value=this.form.mb_intercept_date.defaultValue; }">
	...
	?>
    <tr>
        <th scope="row"><label for="mb_<?php echo $i ?>"><?php echo $title ?></label></th>
