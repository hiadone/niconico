...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
           ...
           <select name="cp_method" id="cp_method">
				...
				<option value="10"<?php echo get_selected('10', $cp['cp_method']); ?>>포인트적립</option>
           </select>
        </td>
    </tr>
    <tr id="tr_cp_target">
	...
	<?php /* 포인트쿠폰 양식 추가 */ ?>
	<tr id="point_cp1">
        <th scope="row"><label for="cp_type">쿠폰코드</label></th>
        <td>
           <?php echo help("포인트적립형 쿠폰은 수동으로 쿠폰코드를 입력하셔야 합니다."); ?>
		   <?php
		   		$readonly = '';
				if ($w == 'u') {
					$readonly = 'readonly';
		   ...
		   ?>
           <input type="text" name="cp_id_tmp" value="<?php echo $cp['cp_id']; ?>" id="cp_id_tmp" class="frm_input" size="50" <?php echo $readonly; ?>>
		   <?php if ($w == '') { ?>
		   <button type="button" id="sch_cp_id" class="btn_frmline">중복체크</button>
		   <br><span id="checkCpIdResult">중복체크 하세요.</span>
		   <?php } ?>
        </td>
    </tr>
	<?php /****************/ ?>
...
$(function() {
    ...
    $("#tr_cp_trunc").hide();
    <?php } ?>
	<?php if($cp['cp_method'] == 10) { ?>
    $("#point_cp1").show();
	$("#tr_cp_target").hide();
	$("#tr_cp_target").find("input").attr("required", false).removeClass("required");
	$("#cp_price_unit").text("포인트");
    $("#cp_price_unit").closest("tr").find("label").text("적립포인트");
    <?php } else { ?>
	$("#point_cp1").hide();
	<?php } ?>
	...
	$("#sch_cp_id").click(function() {
		$.ajax({
			...
			data : {
				action : 'checkCouponNumber',
				couponNumber: $("#cp_id_tmp").val()
				//couponNumber: $("#coupon1").val() +"-"+ $("#coupon2").val() +"-"+ $("#coupon3").val() +"-"+ $("#coupon4").val()
			},
			success: function (response) {
				$("#checkCpIdResult").html(response.msg);			
			},
