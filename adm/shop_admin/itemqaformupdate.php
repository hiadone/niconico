...
if ($w == "u")
{
    ...
    if(trim($iq_answer)) {
        $sql = " select a.iq_name, a.iq_email, a.iq_hp, b.it_name
		...
		if ($row['iq_hp']) {
			/* 뿌리오 알림톡 보내기 */
			$hp = $row['iq_hp'];
			$content = getTemplate('product_cs');
			$content = replaceStrPPurio($content);

			$content = str_replace("#{mbr_name}", $row['iq_name'], $content);

			sendPPurio(str_replace("-", "", $hp), addslashes($content), 'product_cs', 5);
		}
    }
