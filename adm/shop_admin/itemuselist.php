...
<div class="local_ov01 local_ov">
	...
	<button id="excelDownloadBtn" style="background-color:#288440;border:0;border-radius:4px;color:#fff;padding:7px 10px;">엑셀다운로드</button>
</div>
...
<div class="tbl_frm01 tbl_wrap">
    <table>
		<tr>
			<th scope="row">캠페인아이디</th>
			<td>
				<input type="text" name="" id="campaignId" class="frm_input" value="<?php echo $default['de_campaignId']; ?>"/> <input type="button" value="확인" class="btn_submit btn" id="saveCampaignIdBtn"> <span>영문 또는 숫자 조합만 입력 ex)</span>
				https://www.niconicomall.com/shop/item.php?it_id=EV30300002<span style="color:red;">&event_id=캠페인아이디값</span>
			</td>
		</tr>
		<tr>
			<th scope="row">단어필터링</th>
			<td>
				<?php echo help('입력된 단어가 포함된 내용은 게시할 수 없습니다. 단어와 단어 사이는 ,로 구분합니다.') ?>
                <textarea name="de_filterUselist" id="de_filterUselist" rows="7"><?php echo $default['de_filterUselist'] ?></textarea>
			</td>
		</tr>
		<tr>
			<td colspan="2" style="text-align:center;">
				<input type="button" value="확인" class="btn_submit btn" id="saveUselistBtn" accesskey="s">
			</td>
		</tr>
	</table>
</div>
...
$(function(){
	...
	$("#saveUselistBtn").click(function(){
		var conf = confirm("단어 필터링을 수정하시겠습니까?");
		if (conf)
		{
			var ajaxActionURL = 'ajax.etc.php';
			$.ajax({
				url			: ajaxActionURL,
				type		: 'POST',
				dataType	: 'json',
				...
				data : {
					action				: 'saveDefaultItemUselist',
					de_filterUselist	: $("#de_filterUselist").val()
				},
	...
	$("#saveCampaignIdBtn").click(function(){
		var conf = confirm("캠페인 아이디를 등록하시겠습니까?");
		if (conf)
		{
			var ajaxActionURL = 'ajax.etc.php';
			$.ajax({
				...
				contentType	: 'application/x-www-form-urlencoded; charset=utf-8',
				data : {
					action				: 'saveCampaignId',
					de_campaignId		: $("#campaignId").val()
				},
				success: function (response) {
					if (response.code == 1)
					{
	...
	$("#excelDownloadBtn").click(function(){
		$.download('excelPrint.php', 'action=itemUseList&sfl=<?php echo $sfl; ?>&sca=<?php echo $sca; ?>&stx=<?php echo $stx; ?>', 'post');
	});
});
</script>
