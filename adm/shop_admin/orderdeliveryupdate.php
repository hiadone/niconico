...
if($_FILES['excelfile']['tmp_name']) {
    ...
    */
    ...
    for ($k = 2; $k <= $data->sheets[0]['numRows']; $k++) {
		...
		//$data->sheets[0]['cellsInfo'][$k][11]['type'] = 'date';
		$od_invoice_time     = trim($data->sheets[0]['cells'][$k][11]);
		...
		if ($od_invoice_time) {
			$delivery['invoice_time'] = date("Y-m-d H:i:s", strtotime(str_replace('/','-', $od_invoice_time)) - 86400);
		} else {
			$delivery['invoice_time'] = G5_TIME_YMDHIS;
		}
