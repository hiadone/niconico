...
if ($csv == 'csv')
{
    ...
    $sql = " SELECT a.od_id, od_b_zip1, od_b_zip2, od_b_addr1, od_b_addr2, od_b_addr3, od_b_addr_jibeon, od_b_addr_eng, od_b_addr_eng2, od_b_name, od_b_tel, od_b_name_en, od_b_num, od_b_hp, b.it_name, c.it_name_jp, ct_qty, b.it_id, a.od_id, od_memo, od_invoice, b.ct_option, b.ct_send_cost, b.it_sc_type
    ...
    echo iconv('utf-8', 'euc-kr', "우편번호,한글주소,영문주소,이름, 영문이름,전화1,전화2,개인통관번호,상품명(한글), 상품명(일어),수량,선택사항,배송비,상품코드,주문번호,운송장번호,전하실말씀\n");
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
		...
		echo '"'.$row['od_b_addr_eng2'] .' '. $row['od_b_addr_eng'].'"'.',';
        echo '"'.$row['od_b_name'].'"'.',';
		echo '"'.$row['od_b_name_en'].'"'.',';
        //echo '"'.multibyte_digit((string)$row[od_b_tel]).'"'.',';
        //echo '"'.multibyte_digit((string)$row[od_b_hp]).'"'.',';
        echo '"'.(string)' +82 '. substr(conv_telno($row['od_b_tel']), 1) . '"'.',';
        echo '"'.(string)' +82 '. substr(conv_telno($row['od_b_hp']), 1) . '"'.',';
		echo '"'.$row['od_b_num'] . '"'.',';
        echo '"'.preg_replace("/\"/", "&#034;", $row['it_name']) . '"'.',';
		echo '"'.preg_replace("/\"/", "&#034;", $row['it_name_jp']) . '"'.',';
        echo '"'.$row['ct_qty'].'"'.',';
        echo '"'.$row['ct_option'].'"'.',';
        echo '"'.$ct_send_cost.'"'.',';
        echo '"\''.$row['it_id'].'\'"'.',';
...
if ($csv == 'xls')
{
   ...
   $sql = " SELECT a.od_id, od_b_zip1, od_b_zip2, od_b_addr1, od_b_addr2, od_b_addr3, od_b_addr_jibeon, od_b_addr_eng, od_b_addr_eng2, od_b_name, od_b_name_en, od_b_num, od_b_tel, od_b_hp, a.od_time, a.od_id, od_memo, od_invoice, a.od_receipt_point, a.od_receipt_price, a.od_settle_case, a.od_send_cost, 
			b.it_name, b.it_id, b.ct_option, b.ct_send_cost, b.it_sc_type, ct_qty, 
			c.it_name_jp, c.it_weight, c.it_price, c.it_jancode 
               FROM {$g5['g5_shop_order_table']} a, {$g5['g5_shop_cart_table']} b, {$g5['g5_shop_item_table']} c
              where a.od_id = b.od_id and b.it_id = c.it_id  ";
    if ($case == 1) // 출력기간
        $sql .= " and a.od_time between '$fr_date 00:00:00' and '$to_date 23:59:59' ";
    ...
    // Put Excel data
	$data = array('판매채널', '주문일자', '주문번호', '결제수단', '수취인 이름', '영문이름', '개인통관고유부호', '수취인연락처1', '수취인연락처2', '우편번호', '배송지', '영문배송지', '배송메시지', '주문 합계액', '상품 합계액', '전체배송비', '포인트사용액', '상품코드PID', 'JANcode', '상품명일본어', '상품명한글', '선택사항', '수량', '중량', '총중량', '판매가', '총판매가', '구입처', '발송일자', '송장번호', '실제무게', '해외배송비', '공급가(엔화)', '총공급가(엔화)');

	$data2 = array('チャネル', '注文日', '注文番號', '決濟方法', '受取人', '英文名', '個人通關固有番號', '連絡先１', '連絡先２', '郵便番號', '住所', '住所(英語)', '備考', '注文合計額', '商品合計額', '配送料', 'ポイント使用', '商品PID番號', 'JANcode', '品名(日本)', '品名(韓國)', 'オプション', '數量', '重量', '全重量', '商品金額', '全商品金額', '仕入れ先', '運送日', '運送番號', '實 重量', '直接配送料', '供給價', '全 供給價');

    $data = array_map('iconv_euckr', $data);
	$data2 = array_map('iconv_euckr', $data2);
    ...
    for($i=2; $row=sql_fetch_array($result); $i++)
    {
		...
		$worksheet->write($i, $cell++, ' ');
		$worksheet->write($i, $cell++, date("m/d/Y", strtotime($row['od_time'])));
		$worksheet->write($i, $cell++, (string)' '.$row['od_id']);
		$worksheet->write($i, $cell++, (string)' '.$row['od_settle_case']);
		$worksheet->write($i, $cell++, $row['od_b_name']);
		$worksheet->write($i, $cell++, $row['od_b_name_en']);
		$worksheet->write($i, $cell++, (string)' '.$row['od_b_num']);
		$worksheet->write($i, $cell++, (string)' '. substr($row['od_b_tel'], 0));
        $worksheet->write($i, $cell++, (string)' '. substr($row['od_b_hp'], 0));
		$worksheet->write($i, $cell++, ' '.$row['od_b_zip1'].$row['od_b_zip2']);
		$worksheet->write($i, $cell++, print_address($row['od_b_addr1'], $row['od_b_addr2'], $row['od_b_addr3'], $row['od_b_addr_jibeon']));
		$worksheet->write($i, $cell++, $row['od_b_addr_eng2'] .' '. $row['od_b_addr_eng']);
		$worksheet->write($i, $cell++, $row['od_memo']); // 배송메시지
		//$worksheet->write($i, $cell++, (int)$row['od_receipt_price']); // 주문합계액
		$worksheet->write($i, $cell++, (int)$sum2['price']+$row['od_send_cost']-$row['od_receipt_point']); // 주문합계액 뉴버전
		//$worksheet->write($i, $cell++, (int)($row['it_price']*$row['ct_qty'])); // 상품 함계액
		$worksheet->write($i, $cell++, (int)$sum2['price']); // 상품함계액 뉴버전
		$worksheet->write($i, $cell++, (int)$row['od_send_cost']);
		$worksheet->write($i, $cell++, (int)$row['od_receipt_point']);
		$worksheet->write($i, $cell++, (string)' '.$row['it_id']);
		$worksheet->write($i, $cell++, (string)' '.$row['it_jancode']);
		$worksheet->write($i, $cell++, $row['it_name_jp']);
		$worksheet->write($i, $cell++, $row['it_name']);
		$worksheet->write($i, $cell++, (string)' '.$row['ct_option']);
		$worksheet->write($i, $cell++, (int)$row['ct_qty']);
		$worksheet->write($i, $cell++, (int)$row['it_weight']);
		$worksheet->write($i, $cell++, ($row['it_weight']*$row['ct_qty']));
		$worksheet->write($i, $cell++, (int)$row['it_price']);
		//$worksheet->write($i, $cell++, (int)$sum2['price']);  //총판매가
		$worksheet->write($i, $cell++, (int)($row['it_price']*$row['ct_qty'])); // 총판매가 뉴버전
		$worksheet->write($i, $cell++, ' ');
		$worksheet->write($i, $cell++, ' ');
		$worksheet->write($i, $cell++, ' ');
		$worksheet->write($i, $cell++, ' ');
	...
	header("Content-Type: application/x-msexcel; name=\"orderlist-".date("ymd", time()).".xls\"; charset=UTF-8");
    header("Content-Disposition: inline; filename=\"orderlist-".date("ymd", time()).".xls\"; charset=UTF-8");
    $fh=fopen($fname, "rb");
    fpassthru($fh);
    unlink($fname);
