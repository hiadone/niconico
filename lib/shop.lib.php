...
class item_list
{
	...
	// 관리자만 노출하는 상품일 경우 1로 표시할 것
    protected $isOnlyAdmin = 0;
    ...
    protected $view_best_num = false;           // best 순위
	...
	// 관리자만 노출시키기 (1로 넣음 글케 댄다)
    function set_isOnlyAdmin($isOnlyAdmin) {
    ...
    function run() {
        ...
        if ($this->query) {
			...
			if ($this->isOnlyAdmin == 10) {
                $where[] = " it_isOnlyAdmin IN (0, 2, 10) ";
            } elseif ($this->isOnlyAdmin == 2) { 
				$where[] = " it_isOnlyAdmin IN (0, 2) ";
			} else {
				//$where[] = " it_isOnlyAdmin = '0' ";
				$where[] = " it_isOnlyAdmin IN (0, 2) ";
			}
...
class item_use_list
{
    ...
    // 노출순서
    protected $order_by = "is_type= 'photo' desc, CHARACTER_LENGTH(is_content) desc";
    ...
    // 기본으로 보여지는 필드들
    protected $view_is_subject    = false;       // 상품코드
    protected $view_it_img   = true;        // 상품이미지
    protected $view_it_name  = true;        // 상품명
    protected $view_it_basic = true;        // 기본설명
    protected $view_it_price = true;        // 판매가격
    ...
    function set_list_row($list_row) {
        global $default;
        if ($this->is_mobile) {
            $this->list_row = $list_row ? $list_row : 3;
        } else {
            $this->list_row = $list_row ? $list_row : 5;
        }
        if (!$this->list_row)
            $this->list_row = 1;
    }
    ...
    function run() {
        ...
        if ($this->query) {
            ...
            $where = array();
            array_push($where," is_confirm = '1' ");
            array_push($where," is_status = '1' ");
            array_push($where," is_best = '1' ");
...
function get_it_image($it_id, $width, $height=0, $anchor=false, $img_id='', $img_alt='', $is_crop=false)
{
    ...
    if($thumb) {
		...
		$img = '<img src="'.$file_url.'?v='.date("YmdHi", time()).'" width="'.$width.'" height="'.$height.'" alt="'.$img_alt.'"';
    } else {
...
function item_icon($it)
{
    ...
    if ($it['it_type1'])
        $icon .= '<span class="shop_icon shop_icon_1">HOT</span>';

    if ($it['it_type2'])
        $icon .= '<span class="shop_icon shop_icon_2">신상품</span>';

    if ($it['it_type3'])
        $icon .= '<span class="shop_icon shop_icon_3">베스트</span>';

    if ($it['it_type4'])
        $icon .= '<span class="shop_icon shop_icon_4">추천</span>';

    if ($it['it_type5'])
        $icon .= '<span class="shop_icon shop_icon_5">SALE</span>';
...
function get_order_info($od_id)
{
	...
	for ($i=0; $row=sql_fetch_array($result); $i++)
	{
		...
		// 무게는 수량 * 개별무게
		$tWeight = $row['it_weight'] * $sum2['qty'];
		$tot_weight		+= $tWeight;
	}
...
function check_itemuse_write($it_id, $mb_id, $close=true)
{
    ...
    if(!$is_admin && $default['de_item_use_write'])
    {
        ...
        if($row['cnt'] == 0)
        {            
            return '사용후기는 주문이 완료된 경우에만 작성하실 수 있습니다.';            
        }
    }
    return false;
}
...
function getDeliveryCost($tot_weight) {
	if ($tot_weight >= 0 && $tot_weight <= 1) {
		$send_cost = 500;
	} elseif ($tot_weight > 1 && $tot_weight <= 500) {
		$send_cost = 8500;
	} elseif ($tot_weight > 500 && $tot_weight <= 1000) {
		$send_cost = 11000;
	} elseif ($tot_weight > 1000 && $tot_weight <= 1500) {
		$send_cost = 12000;
	} elseif ($tot_weight > 1500 && $tot_weight <= 2000) {
		$send_cost = 13500;
	} elseif ($tot_weight > 2000 && $tot_weight <= 2500) {
		$send_cost = 15000;
	} elseif ($tot_weight > 2500 && $tot_weight <= 3000) {
		$send_cost = 17000;
	} elseif ($tot_weight > 3000 && $tot_weight <= 3500) {
		$send_cost = 18500;
	} elseif ($tot_weight > 3500 && $tot_weight <= 4000) {
		$send_cost = 21000;
	} elseif ($tot_weight > 4000 && $tot_weight <= 4500) {
		$send_cost = 22500;
	} elseif ($tot_weight > 4500 && $tot_weight <= 5000) {
		$send_cost = 25500;
	} elseif ($tot_weight > 5000 && $tot_weight <= 5500) {
		$send_cost = 26500;
	} elseif ($tot_weight > 5500 && $tot_weight <= 6000) {
		$send_cost = 28500;
	} elseif ($tot_weight > 6000 && $tot_weight <= 6500) {
		$send_cost = 31500;
	} elseif ($tot_weight > 6500 && $tot_weight <= 7000) {
		$send_cost = 32500;
	} elseif ($tot_weight > 7000 && $tot_weight <= 7500) {
		$send_cost = 33500;
	} elseif ($tot_weight > 7500 && $tot_weight <= 8000) {
		$send_cost = 35500;
	} elseif ($tot_weight > 8000 && $tot_weight <= 8500) {
		$send_cost = 37500;
	} elseif ($tot_weight > 8500 && $tot_weight <= 9000) {
		$send_cost = 39500;
	} elseif ($tot_weight > 9000 && $tot_weight <= 9500) {
		$send_cost = 41500;
	} elseif ($tot_weight > 9500 && $tot_weight <= 10000) {
		$send_cost = 43500;
	} elseif ($tot_weight > 10000 && $tot_weight <= 11000) {
		$send_cost = 47000;
	} elseif ($tot_weight > 11000 && $tot_weight <= 12000) {
		$send_cost = 51000;
	} elseif ($tot_weight > 12000 && $tot_weight <= 13000) {
		$send_cost = 54000;
	} elseif ($tot_weight > 13000 && $tot_weight <= 14000) {
		$send_cost = 58000;
	} else {
		$send_cost = 58000;
	}
