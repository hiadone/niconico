...
<div id="sod_bsk" class="od_prd_list">
    ...
    <div class="tbl_head03 tbl_wrap">
        ...
        <tr>
			...
			<th scope="col">무게</th>
            <th scope="col">배송비</th>
            <th scope="col" style="width:120px;">소계</th>
        </tr>
        </thead>
        <tbody>
        <?php
        ...
        //$sql .= " order by a.ct_id asc, b.it_id asc ";
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++)
        {
			...
			/*
			if ($maxDeliveryDegree > 1) {

				for ($ccc = 1; $ccc <= $maxDeliveryDegree; $ccc++) {
					$remainQty = $sum['qty'] - ($ccc * 6);
					if ($remainQty > 6) {
						$sumQty = 6;
					} else {
						$sumQty = $remainQty;
					}
					$cartInfoArr[$deliveryDegree][$row['it_id']] = [
						'imgTag' => "<a href=\"". shop_item_url($row['it_id']) ."\">". $image ."</a>",
						'sumQty' => $sumQty,
					];
				}
			*/
...
<style>
	...
	#sod_bsk_list .disable td {
		...
		<div class="btn_cart_del" style="color:red;">
		※ 건강기능식품은 관세법상 통관 기준인 1인당 6개까지 구매하실 수 있습니다.<BR>
		※ 1회 통관 시 배송비 제외 상품금액 150$ (한화 17만원)이내에서 면세입니다. 이를 초과할 시에는 추가 관세가 발생할 수 있습니다.<br>
		※ 국제배송비는 상품 무게에 따라 측정되며 500g 기준 8,500원 입니다. <a href="/bbs/content.php?co_id=delivery" target="_blank"><u>(무게별 배송비 보기)</u></a>
        </div>
    </div>
    ...
    <div id="sod_bsk_tot">
        <ul>
            <li class="sod_bsk_dvr">
                <span>제품 가격</span>
                <strong id="tot_sell_price"><?php echo number_format($tot_sell_price); ?></strong> 원
            </li>

			<li class="sod_bsk_pt">
                <span>총 무게</span>
                <strong id="tot_sell_weight"><?php echo number_format($tot_weight); ?></strong> g
            </li>
			
			<li class="sod_bsk_dvr">
                <span>배송비</span>
                <strong id="tot_sell_sendCost"><?php echo number_format($send_cost); ?></strong> 원
            </li>

            <li class="sod_bsk_cnt">
                <span>총계 가격</span>
                <strong id="total_price"><?php echo number_format($tot_price); ?></strong> 원 
            </li>
        </ul>
    </div>
    <?php } ?>
...
$(function() {
	...
	var calcTotalPrice = function(){
		var sellPrice = 0;
		var sellWeight = 0;
		$("tbody .selec_chk").each(function(idx){
		...
		$("#tot_sell_price").html(numberWithCommas(sellPrice));
		$("#tot_sell_weight").html(numberWithCommas(sellWeight));
		$("#tot_sell_sendCost").html(numberWithCommas(totalSendCost));
		$("#total_price").html(numberWithCommas(totalPrice));
	};
	
	$("tbody .selec_chk").click(function(){
		calcTotalPrice();
	});
});
