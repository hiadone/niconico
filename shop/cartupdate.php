...
$windowMode = isset($_POST['windowMode']) ? $_POST['windowMode'] : 0;
...
if ($sw_direct)
{
    ...
    {
    	if ($windowMode == 1) {
			top_goto_url(G5_SHOP_URL."/orderform.php?sw_direct=$sw_direct");
		} else {
    ...
    {
		if ($windowMode == 1) {
			top_goto_url(G5_BBS_URL."/login.php?url=".urlencode(G5_SHOP_URL."/orderform.php?sw_direct=$sw_direct"));
		} else {
...
{	
	if ($windowMode == 1) {
		/* 히든 아이프레임으로 온 것이라 팝업을 띄워준다 */
?>
<script>
parent.addCartComplete();
</script>
<?php
	} else {
