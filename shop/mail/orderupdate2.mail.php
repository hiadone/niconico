...
<div style="margin:30px auto;width:600px;border:10px solid #f7f7f7">
    <div style="border:1px solid #dedede">
        ...
        <?php //for ($i=0; $i<count($list); $i++) { 
			...
			$addTitle = '';
			if (count($list) > 1) {
				$extraCnt = count($list) - 1;
				$addTitle = "외 ". $extraCnt;
			}
		?>
        <tr>
            <th scope="row" style="<?php echo $th_st; ?>">상품명</th>
            <td style="<?php echo $td_st; ?>"><a href="<?php echo shop_item_url($list[$i]['it_id']); ?>" target="_blank" style="text-decoration:none"><span style="display:inline-block;vertical-align:middle"><?php echo $list[$i]['it_simg']; ?></span> <?php echo $list[$i]['it_name']; ?></a> <?php echo $addTitle; ?></td>
        </tr>
        <!-- tr>
        ...
        <?php if ($pg_price > 0 && $od_settle_case == '가상계좌') { ?>
        <tr>
            <th scope="row" style="<?php echo $th_st; ?>">입금하실 계좌</th>
            <td style="<?php echo $td_st; ?>"><?php echo $od_bank_account; ?></td>
        </tr>
        <?php } ?>

        <?php //if (!$od_receipt_point && !$od_receipt_price) { 
				if (!$od_receipt_point && !$pg_price) { 
