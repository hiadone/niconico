...
<div id="sod_addr" class="new_win">
    ...
    <div class="tbl_head03 tbl_wrap">
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            $addr = $row['ad_name'].$sep.$row['ad_tel'].$sep.$row['ad_hp'].$sep.$row['ad_zip1'].$sep.$row['ad_zip2'].$sep.$row['ad_addr1'].$sep.$row['ad_addr2'].$sep.$row['ad_addr3'].$sep.$row['ad_jibeon'].$sep.$row['ad_subject'].$sep.$row['ad_addr_eng'].$sep.$row['ad_addr_eng2'].$sep.$row['ad_b_number'].$sep.$row['ad_name_eng'];
            $addr = get_text($addr);
        ?>
        <tr>
            <td class="td_sbj">
...
$(function() {
    $(".sel_address").on("click", function() {
		...
		f.od_b_addr_eng.value    = addr[10];
		f.od_b_addr_eng2.value   = addr[11];
		f.od_b_num.value         = addr[12];
		f.od_b_name_en.value     = addr[13];
