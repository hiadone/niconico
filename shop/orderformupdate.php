...
if (get_cart_count($tmp_cart_id) == 0) {    // 장바구니에 담기
    ...
    alert('장바구니가 비어 있습니다.\\n\\n이미 주문하셨거나 장바구니에 담긴 상품이 없는 경우입니다. cart id :'. $tmp_cart_id, G5_SHOP_URL.'/cart.php');
}
...
$i_temp_point = (int)str_replace(",", "", $_POST['od_temp_point']);
...
$od_hp            = format_phone(clean_xss_tags($od_hp));
...
$od_addr_eng      = clean_xss_tags($_POST['od_addr_eng']);
$od_addr_eng2     = clean_xss_tags($_POST['od_addr_eng2']);
$od_addr_jibeon   = preg_match("/^(N|R)$/", $od_addr_jibeon) ? $od_addr_jibeon : '';
$od_b_name        = clean_xss_tags($od_b_name);
$od_b_name_en     = clean_xss_tags($_POST['od_b_name_en']);
$od_b_tel         = clean_xss_tags($od_b_tel);
$od_b_hp          = format_phone(clean_xss_tags($od_b_hp));
$od_b_addr1       = clean_xss_tags($od_b_addr1);
$od_b_addr2       = clean_xss_tags($od_b_addr2);
$od_b_addr3       = clean_xss_tags($od_b_addr3);
$od_b_addr_eng    = clean_xss_tags($_POST['od_b_addr_eng']);
$od_b_addr_eng2    = clean_xss_tags($_POST['od_b_addr_eng2']);
...
$od_b_num         = clean_xss_tags($_POST['od_b_num']);
...
$sql = " insert {$g5['g5_shop_order_table']}
            set od_id             = '$od_id',
				...
				od_addr_eng		  = '$od_addr_eng',
				od_addr_eng2	  = '$od_addr_eng2',
                od_b_name         = '$od_b_name',
				od_b_name_en      = '$od_b_name_en',
                od_b_tel          = '$od_b_tel',
                od_b_hp           = '$od_b_hp',
                od_b_zip1         = '$od_b_zip1',
                od_b_zip2         = '$od_b_zip2',
				...
				od_b_addr_eng	  = '$od_b_addr_eng',
				od_b_addr_eng2	  = '$od_b_addr_eng2',
...
if ($is_member && $od_receipt_point) {
	...
	/* 뿌리오 알림톡 보내기 */
	if ($od_hp) {
		$content = getTemplate('point_deduct_2');
		$content = replaceStrPPurio($content);

		$content = str_replace("#{mbr_name}", $od_name, $content);
		$content = str_replace("#{dd_point}", $od_receipt_point, $content);

		sendPPurio(str_replace("-", "", $od_hp), $content, 'point_deduct_2', 1);
	}
}
...
/* 뿌리오 알림톡 보내기 */
if ($od_hp && $od_settle_case == '가상계좌') {
	$content = getTemplate('order_done_2');
	$content = replaceStrPPurio($content);
	...
	$content = str_replace("#{bankaccount}", $od_bank_account, $content);

	sendPPurio(str_replace("-", "", $od_hp), $content, 'order_done_2', 2);
}

if ($od_hp && $od_status == "입금") {
    ...
    if($row['ad_id']){
        $sql = " update {$g5['g5_shop_order_address_table']}
                      set ad_default = '$ad_default',
						  ...
						  ad_name_eng = '$od_b_name_en',
						  ad_b_number = '$od_b_num'
                    where mb_id = '{$member['mb_id']}'
                      and ad_id = '{$row['ad_id']}' ";
    } else {
        $sql = " insert into {$g5['g5_shop_order_address_table']}
                    set mb_id       = '{$member['mb_id']}',
                        ...
                        ad_addr3    = '$od_b_addr3',
                        ad_jibeon   = '$od_b_addr_jibeon',
						ad_addr_eng = '$od_b_addr_eng',
						ad_addr_eng2 = '$od_b_addr_eng2',
						ad_b_number = '$od_b_num',
						ad_name_eng = '$od_b_name_en'
						";
    }
...
goto_url(G5_SHOP_URL.'/orderinquiryview.php?od_id='.$od_id.'&amp;uid='.$uid.'&amp;fpage=order');
?>
