...
/* 뿌리오 알림톡 보내기 */
if ($w == "" && $receive_number) {
	$content = getTemplate('signup_2');
	$content = replaceStrPPurio($content);

	$content = str_replace("#{mbr_name}", $mb_name, $content);
	$content = str_replace("#{mbr_id}", $mb_id, $content);

	sendPPurio(str_replace("-", "", $receive_number), $content, 'signup_2');
}
//----------------------------------------------------------
// SMS 문자전송 끝
//----------------------------------------------------------
