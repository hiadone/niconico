...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    if (file_exists($bimg))
    {
        ...
        if ($row['bn_url'][0] == '#')
            ...
            $banner .= '<a href="'.G5_SHOP_URL.'/bannerhit.php?bn_id='.$row['bn_id'].'"'.$bn_new_win.' target="_blank">';
        }
        $item_html .= $banner.'<img src="'.G5_DATA_URL.'/banner/'.$row['bn_id'].'?v='.date("YmdHi", time()) .'" width="'.$size[0].'" alt="'.get_text($row['bn_alt']).'"'.$bn_border.'>';
        if($banner)
            $item_html .= '</a>';
        $item_html .= '</div>';
    }
